...
totalHgenera_before5<-genus_rel_abundH%>%group_by(taxon)%>%distinct(taxon)
...
#Across all harvest samples, the 5 most common bacterial genera were: #pick out top five genera BY AVG RELATIVE ABUNDANCE
top<-join_taxon_rel_abundH%>%group_by(taxon,mean)%>%filter(duplicated(taxon) == FALSE)
top <-top[order(-top$mean),]
table10<-head(top, 6)
table10<-table10%>%select(taxon,mean)
...
Hotucounts<-as.data.frame(H@otu_table)
Hotucounts_rowsums<-as.data.frame(rowSums(Hotucounts))
Hotucounts_rowsums <-Hotucounts_rowsums%>% arrange(desc(rowSums(Hotucounts)))
Hotucounts_rowsums_top3<-head(Hotucounts_rowsums, 3)
Hotucounts_rowsums_top3<-Hotucounts_rowsums_top3%>%rownames_to_column("OTU")
Hotucounts_rowsums_top3_taxa<-left_join(Hotucounts_rowsums_top3,taxH, by="OTU")
...
plot_H_nmds <- ggplot(plot_dfH, aes(x = NMDS1, y = NMDS2, color = Month, shape = Location)) +
  ...
  stat_ellipse(linetype = 2, size = .25) +
  theme_classic()
plot_H_nmds

ggsave("H_FUNGAL_NMDS.tiff", units="in", width=8, height=6, dpi=300, compression = 'lzw')
...
genus_rel_abundDI <- taxa.meta.sumDI %>%
  ...
  ungroup()
#how many genera are there total?
totalDIgenera_before5<-genus_rel_abundDI%>%group_by(taxon)%>%distinct(taxon)

genus_rel_abundDI_AVG <- genus_rel_abundDI%>%group_by(taxon) %>%mutate(avgrelab=mean(rel_abund))%>%distinct(taxon,avgrelab)
genus_rel_abundDI_AVG_top5<-head(arrange(genus_rel_abundDI_AVG,desc(avgrelab)), n = 5)
...
palNMDS <- c( "#1F78B4", "#B2DF8A","#FB9A99","#A6CEE3","#E31A1C" ,"#FDBF6F","#33A02C","#FF7F00", "#CAB2D6","turquoise","black","darkorchid4")

plot_HDI_nmds <- ggplot(HDIplot_df, aes(x = NMDS1, y = NMDS2, color = Day, shape = Location)) +
  geom_point(size = 3, alpha = 0.8) +
  scale_color_manual(values = palNMDS) +
  stat_ellipse(linetype = 2, size = .25) +
  theme_classic()
plot_HDI_nmds
ggsave("HDI_FUNGAL_NMDS.tiff", units="in", width=8, height=6, dpi=300, compression = 'lzw')
...
plot_fiveday_nmds <- ggplot(fivedayplot_df, aes(x = NMDS1, y = NMDS2, color = Day, shape = Location)) +
  ...
  theme_classic()
plot_fiveday_nmds
ggsave("FIVEDAY_FUNGAL_NMDS.tiff", units="in", width=8, height=6, dpi=300, compression = 'lzw')
...
#H average relative abundance
table10
colnames(table10)[2] <- "Average Relative Abundance at Harvest"

#DI average relative abundance
genus_rel_abundDI_AVG_top5
colnames(genus_rel_abundDI_AVG_top5)[2] <- "Average Relative Abundance at Day Initial"
...
write.csv(table10,"/local1/workdir1/tw488/CIDA/For_Publication/ITS/Table10_top5avggenera_08192024/H", row.names = TRUE)
write.csv(genus_rel_abundDI_AVG_top5,"/local1/workdir1/tw488/CIDA/For_Publication/ITS/Table10_top5avggenera_08192024/DayI", row.names = TRUE)
